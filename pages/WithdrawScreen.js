...
// Mask account number like: XXXXXXXXXX8745
...
// ─── Web status badge styles (same logic, orange theme) ──────────────────────
const webStatusStyle = (status) => {
  switch (status?.toLowerCase()) {
    case 'approved': return { backgroundColor: '#e6faf3' };
    case 'rejected': return { backgroundColor: '#fff0f5' };
    default:         return { backgroundColor: '#fff4e0' };
  }
};
const webStatusTextStyle = (status) => {
  switch (status?.toLowerCase()) {
    case 'approved': return { color: '#14b87a' };
    case 'rejected': return { color: '#e8603c' };
    default:         return { color: '#c47a00' };
  }
};
...
export default function WithdrawScreen({ navigation }) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // WEB LAYOUT — Centered card, orange theme, Image-1 style
  // ═══════════════════════════════════════════════════════════════════════════
  if (isWeb && !isMobileWeb) {
    return (
      <View style={WithdrawStyles.root}>
        <ScrollView
          style={{ flex: 1 }}
          contentContainerStyle={WithdrawStyles.webPage}
          showsVerticalScrollIndicator={false}
          keyboardShouldPersistTaps="handled"
        >
          {/* ── Back Button ── */}
          <View style={WithdrawStyles.webBackBtn}>
            <TouchableOpacity
              onPress={() => navigation.navigate('QuickMenu')}
              style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}
            >
              <Feather name="arrow-left" size={18} color={ORANGE} />
              <Text style={WithdrawStyles.webBackBtnText}>Back</Text>
            </TouchableOpacity>
          </View>

          {/* ── Main Form Card ── */}
          <View style={WithdrawStyles.webCard}>
            <View style={WithdrawStyles.webCardBody}>

              {/* ── Available Balance ── */}
              <View style={WithdrawStyles.webBalanceCard}>
                <Feather name="credit-card" size={20} color={ORANGE} />
                <View style={WithdrawStyles.webBalanceInfo}>
                  <Text style={WithdrawStyles.webBalanceLabel}>Available Balance</Text>
                  <Text style={WithdrawStyles.webBalanceValue}>
              ...
              {/* Success */}
              {successMessage ? (
                <Text style={WithdrawStyles.webSuccessText}>{successMessage}</Text>
              ) : null}

              {/* ── Section Title ── */}
              <Text style={WithdrawStyles.webSectionTitle}>WITHDRAW EARNING</Text>

              {/* ── Meta Row: Transfer mode + acc number ── */}
              <View style={WithdrawStyles.webMetaRow}>
                <View style={WithdrawStyles.webMetaItem}>
                  <Text style={WithdrawStyles.webMetaLabel}>Transfer mode</Text>
                  <Text style={WithdrawStyles.webMetaValue}>
                    {form.payment_mode === 'upi' ? 'UPI' : 'Bank'}
                  </Text>
                </View>
                {form.payment_mode === 'bank' && form.account_number ? (
                  <View style={WithdrawStyles.webMetaItem}>
                    <Text style={WithdrawStyles.webMetaLabel}>Acc number</Text>
                    <Text style={WithdrawStyles.webMetaValue}>
                  ...
                  <View style={WithdrawStyles.webMetaItem}>
                    <Text style={WithdrawStyles.webMetaLabel}>UPI ID</Text>
                    <Text style={WithdrawStyles.webMetaValue}>{form.upi_id}</Text>
                  </View>
                ) : null}
              </View>

              {/* ── Amount ── */}
              <View style={WithdrawStyles.webInputGroup}>
                <Text style={WithdrawStyles.webInputLabel}>Enter amount</Text>
                <TextInput
                  style={[WithdrawStyles.webInput, errors.amount && WithdrawStyles.webInputError]}
                  value={form.amount}
                ...
                />
                {errors.amount ? (
                  <Text style={WithdrawStyles.webErrorText}>{errors.amount}</Text>
                ) : null}
              </View>

              {/* ── Transfer full amount ── */}
              <TouchableOpacity style={WithdrawStyles.webCheckRow} onPress={handleTransferFull}>
                <View style={[WithdrawStyles.webCheckBox, form.transferFull && WithdrawStyles.webCheckBoxActive]}>
                  {form.transferFull && <Feather name="check" size={11} color="#fff" />}
                </View>
                <Text style={WithdrawStyles.webCheckLabel}>Transfer full amount</Text>
              </TouchableOpacity>

              {/* ── Payment Mode ── */}
              <View style={WithdrawStyles.webInputGroup}>
                <Text style={WithdrawStyles.webInputLabel}>Payment Mode</Text>
                <View style={WithdrawStyles.webModeRow}>
                  <TouchableOpacity
                    style={[WithdrawStyles.webModeBtn, form.payment_mode === 'bank' && WithdrawStyles.webModeBtnActive]}
                    onPress={() => { handleChange('payment_mode', 'bank'); setErrors({}); }}
                  >
                    <Feather name="home" size={14} color={form.payment_mode === 'bank' ? '#fff' : '#888'} />
                    <Text style={[WithdrawStyles.webModeBtnText, form.payment_mode === 'bank' && WithdrawStyles.webModeBtnTextActive]}>
                      Bank Transfer
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[WithdrawStyles.webModeBtn, form.payment_mode === 'upi' && WithdrawStyles.webModeBtnActive]}
                    onPress={() => { handleChange('payment_mode', 'upi'); setErrors({}); }}
                  >
                    <Feather name="smartphone" size={14} color={form.payment_mode === 'upi' ? '#fff' : '#888'} />
                    <Text style={[WithdrawStyles.webModeBtnText, form.payment_mode === 'upi' && WithdrawStyles.webModeBtnTextActive]}>
                      UPI
                    </Text>
                  </TouchableOpacity>
                </View>
              ...
              {form.payment_mode === 'bank' && (
                <>
                  <View style={WithdrawStyles.webInputGroup}>
                    <Text style={WithdrawStyles.webInputLabel}>Bank Name</Text>
                    <TextInput
                      style={[WithdrawStyles.webInput, errors.bank_name && WithdrawStyles.webInputError]}
                    ...
                    />
                    {errors.bank_name ? (
                      <Text style={WithdrawStyles.webErrorText}>{errors.bank_name}</Text>
                    ) : null}
                  </View>

                  <View style={WithdrawStyles.webInputGroup}>
                    <Text style={WithdrawStyles.webInputLabel}>Account Number</Text>
                    <TextInput
                      style={[WithdrawStyles.webInput, errors.account_number && WithdrawStyles.webInputError]}
                    ...
                    {errors.account_number ? (
                      <Text style={WithdrawStyles.webErrorText}>{errors.account_number}</Text>
                    ) : null}
                  </View>

                  <View style={WithdrawStyles.webInputGroup}>
                    <Text style={WithdrawStyles.webInputLabel}>IFSC Code</Text>
                    <TextInput
                      style={[WithdrawStyles.webInput, errors.ifsc && WithdrawStyles.webInputError]}
                      value={form.ifsc}
                    ...
                    />
                    {errors.ifsc ? (
                      <Text style={WithdrawStyles.webErrorText}>{errors.ifsc}</Text>
                    ) : (
                      <Text style={WithdrawStyles.webHintText}>
              ...
              {form.payment_mode === 'upi' && (
                <View style={WithdrawStyles.webInputGroup}>
                  <Text style={WithdrawStyles.webInputLabel}>UPI ID</Text>
                  <TextInput
                    style={[WithdrawStyles.webInput, errors.upi_id && WithdrawStyles.webInputError]}
                    value={form.upi_id}
                  ...
                  />
                  {errors.upi_id ? (
                    <Text style={WithdrawStyles.webErrorText}>{errors.upi_id}</Text>
                  ) : null}
                </View>
              )}
              ...
              {/* ── Submit ── */}
              <TouchableOpacity
                style={[WithdrawStyles.webSubmitBtn, submitting && WithdrawStyles.webSubmitBtnDisabled]}
                onPress={handleSubmit}
                disabled={submitting}
              >
                <Feather name="arrow-up-circle" size={17} color="#fff" />
                <Text style={WithdrawStyles.webSubmitBtnText}>
          ...
          <View style={WithdrawStyles.webTableCard}>
            <View style={WithdrawStyles.webTableHeader}>
              <Text style={WithdrawStyles.webTableTitle}>Withdrawal Status</Text>
              <Text style={WithdrawStyles.webRecordCount}>
                {totalRecords} record{totalRecords !== 1 ? 's' : ''}
              </Text>
            </View>
            <View style={WithdrawStyles.webTableBody}>
              {loading ? (
                <Text style={WithdrawStyles.webLoadingText}>Loading withdrawal data...</Text>
              ) : withdrawData.requests.length === 0 ? (
                <Text style={WithdrawStyles.webEmptyText}>No withdrawal requests found.</Text>
              ) : (
                <>
                  <View style={WithdrawStyles.webSearchWrap}>
                    <Feather name="search" size={15} color="#ccc" />
                    <TextInput
                      style={WithdrawStyles.webSearchInput}
                      value={searchQuery}
                  ...
                  {totalRecords === 0 ? (
                    <Text style={WithdrawStyles.webEmptyText}>No records match your search.</Text>
                  ) : (
                    <>
                      {pagedRequests.map((item, index) => (
                        <View
                          key={item.id ?? index}
                          style={[
                            WithdrawStyles.webRequestCard,
                            index % 2 === 0 && WithdrawStyles.webRequestCardEven,
                          ]}
                        >
                          <View style={WithdrawStyles.webRequestTopRow}>
                            <Text style={WithdrawStyles.webRequestAmount}>₹{item.amount}</Text>
                            <View style={[WithdrawStyles.statusBadge, webStatusStyle(item.status)]}>
                              <Text style={[WithdrawStyles.statusText, webStatusTextStyle(item.status)]}>
                                {item.status}
                              </Text>
                            </View>
                          </View>

                          <View style={WithdrawStyles.webRequestMetaRow}>
                            <Feather name="credit-card" size={12} color="#ccc" />
                            <Text style={WithdrawStyles.webRequestMeta}>
                              Mode:{' '}
                              <Text style={WithdrawStyles.webRequestMetaBold}>
                          ...
                          {item.payment_mode === 'bank' ? (
                            <>
                              <View style={WithdrawStyles.webRequestMetaRow}>
                                ...
                                <Text style={WithdrawStyles.webRequestMeta}>
                                  Bank: <Text style={WithdrawStyles.webRequestMetaBold}>{item.bank_name}</Text>
                                </Text>
                              </View>
                              <View style={WithdrawStyles.webRequestMetaRow}>
                              ...
                              <View style={WithdrawStyles.webRequestMetaRow}>
                                ...
                                <Text style={WithdrawStyles.webRequestMeta}>
                                  IFSC: <Text style={WithdrawStyles.webRequestMetaBold}>{item.ifsc}</Text>
                                </Text>
                              </View>
                            </>
                          ) : (
                            <View style={WithdrawStyles.webRequestMetaRow}>
                              ...
                              <Text style={WithdrawStyles.webRequestMeta}>
                                UPI ID: <Text style={WithdrawStyles.webRequestMetaBold}>{item.upi_id}</Text>
                              </Text>
                            </View>
                          )}

                          <Text style={WithdrawStyles.webRequestDate}>
                            Requested on: {item.date}
                          </Text>
                        </View>
                      ))}

                      <View style={WithdrawStyles.webPaginationWrap}>
                        <Text style={WithdrawStyles.webPaginationInfo}>
                          Showing{' '}
                          {Math.min((currentPage - 1) * PAGE_SIZE + 1, totalRecords)}–
                          {Math.min(currentPage * PAGE_SIZE, totalRecords)} of {totalRecords}
                        </Text>
                        <View style={WithdrawStyles.webPaginationControls}>
                          <TouchableOpacity
                            style={[WithdrawStyles.webPageBtn, currentPage === 1 && WithdrawStyles.webPageBtnDisabled]}
                          ...
                          {pageNumbers.map((page) => (
                            <TouchableOpacity
                              key={page}
                              style={[WithdrawStyles.webPageBtn, page === currentPage && WithdrawStyles.webPageBtnActive]}
                              onPress={() => goToPage(page)}
                            >
                              <Text style={[WithdrawStyles.webPageBtnText, page === currentPage && WithdrawStyles.webPageBtnTextActive]}>
                                {page}
                              </Text>
                            </TouchableOpacity>
                          ))}
                          <TouchableOpacity
                            style={[WithdrawStyles.webPageBtn, currentPage === totalPages && WithdrawStyles.webPageBtnDisabled]}
