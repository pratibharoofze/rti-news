...
export default function TalukaSelectScreen({
  navigation,
  route,
}) {
  const { login } = useAuth();
  ...
  const [talukaModal, setTalukaModal] =
    useState(false);

  const allowLeaveRef = useRef(false);

  const registrationJustCompleted =
    !fromPremium;
  ...
  const talukaList = selectedDistrict
    ? getTalukas(
        selectedState,
        selectedDistrict
      )
  ...
  const handleComplete = async () => {
    if (!taluka.trim()) {
      alert(
        'Please select or enter your taluka'
      );
      return;
    }
    ...
    if (needsCreateUser) {
      const pending =
        await UserStore.getPendingRegistration();
